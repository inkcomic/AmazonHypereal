...
class HyperealVRPlayspace : public CFlowBaseNode<eNCT_Singleton>
{
        enum INPUTS
        {
            Activate = 0,
    ...
    public:
        ...
        HyperealVRPlayspace(SActivationInfo* actInfo)
        {
        }
        ...
        void ProcessEvent(EFlowEvent event, SActivationInfo* actInfo) override
        {
            switch (event)
            {
                case eFE_Activate:
                {
                    HyperealVRRequests::Playspace playspace;
                    EBUS_EVENT(HyperealVRRequestBus, GetPlayspace, playspace);
                    ...
                    if (playspace.isValid)
                    {
                        ...
                        Vec3 center = viewParams->position;
...
REGISTER_FLOW_NODE("VR:HyperealVR:Playspace", HyperealVRPlayspace);
} // namespace HyperealVR
