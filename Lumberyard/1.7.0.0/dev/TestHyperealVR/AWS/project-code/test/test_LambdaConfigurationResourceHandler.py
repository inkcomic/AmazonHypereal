...
# TODO: turn this stuff into unit tests
...
# #e need client objects created by the custom resource handler to use this 
# region and profile.
boto3.setup_default_session(region_name=TEST_REGION, profile_name=TEST_PROFILE)
...
class TestLambdaConfigurationResourceHandler(unittest.TestCase):
    ...
    def test_integration_inject_settings_python(self):
        ...
        bucket = TEST_NAME_PREFIX + str(int(time() * 1000))
    ...
    def test_integration_create_update_delete_role(self):
        ...
        reload(LambdaConfigurationResourceHandler) # reset global AWS clients
        reload(role_utils) # reset global AWS clients
        
        with mock.patch.object(stack_info,'ResourceGroupInfo') as mock_ResourceGroupInfo:
    ...
    def _create_role_test_stack(self):
        ...
        cf = session.client('cloudformation', region_name=TEST_REGION)

        # TODO: turn this into an unit test
        stack_name = TEST_NAME_PREFIX + str(int(time() * 1000))
