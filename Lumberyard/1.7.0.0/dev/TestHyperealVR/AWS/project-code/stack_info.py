...
def get_cloud_formation_client(stack_arn):
    ...
    return discovery_utils.CloudFormationClientWrapper(boto3.client('cloudformation', region_name=region))

s3 = log.ClientWrapper(boto3.client('s3'))

class ResourceInfo(object):

    def __init__(self, stack_arn, resource_summary, client):
    ...
    def __detail(self):
        if not self.__resource_detail :
            res = self.client.describe_stack_resource(StackName=self.stack_arn, LogicalResourceId=self.logical_id)
    ...
    def metadata(self):
        if not self.__metadata:
            ...
            try:
                ...
                raise ValidationError('Could not parse stack {} resource {} metadata {}: {}'.format(self.stack_arn, self.logical_id, metadata_string, e))
        return self.__metadata

    def get_cloud_canvas_metadata(self, *names):
        result = self.metadata.get('CloudCanvas', None)
        for name in names:
            if result:
...
class ResourceInfoList(list):
    ...
    def __find_resource(self, attribute, value, expected_type, optional):
        ...
        if not optional:
            raise ValidationError('There is no {} resource in stack {}.'.format(value, self.stack_arn))
...
class ParametersDict(dict):
    ...
    def __getitem__(self, key):
        value = self.get(key)
        if not value:
            raise ValidationError('The {} stack has no {} parameter. This parameter is required.'.format(self.__stack_arn, key))
        return value
...
class StackInfo(object):
    ...
    @property
    def stack_name(self):
        return discovery_utils.get_stack_name_from_stack_arn(self.stack_arn)
    ...
    @property
    def account_id(self):
        return discovery_utils.get_account_id_from_stack_arn(self.stack_arn)
    ...
    def client(self):
        if self.__client is None:
            self.__client = log.ClientWrapper(boto3.client('cloudformation', region_name=self.region))
        return self.__client

    @property
    def resources(self):
        if self.__resources is None:
            resources = ResourceInfoList(self.stack_arn)
            res = self.client.list_stack_resources(StackName=self.stack_arn)
            for resource_summary in res['StackResourceSummaries']:
                resources.append(ResourceInfo(self.stack_arn, resource_summary, self.client))
            while('NextToken' in res):
                ...
                for resoruce_summary in res['StackResourceSummaries']:
                    resources.append(ResourceInfo(resource_summary))
...
class ProjectInfo(StackInfo):
    ...
    def deployments(self):
        if self.__deployment_infos is None:
            deployment_infos = []
            for deployment_name, deployment_settings in self.project_settings.get('deployments', {}).iteritems():
...
def get_stack_info(stack_arn):
    ...
    if not stack_type:
        raise ValidationError('The stack {} is not a Lumberyard Cloud Canvas managed stack.'.format(stack_arn))
    ...
    if stack_type == StackInfo.STACK_TYPE_RESOURCE_GROUP:
        ...
        raise ValidationError('The stack {} has an unexpected Lumberyard Cloud Canvase managed stack type: {}'.format(stack_arn, stack_type))
