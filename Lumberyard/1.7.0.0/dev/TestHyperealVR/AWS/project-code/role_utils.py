...
def _make_resource_statement(cf, stack_arn, policy_name, logical_resource_name, policy_metadata_filter):
    ...
    res = cf.describe_stack_resource(StackName=stack_arn, LogicalResourceId=logical_resource_name)
    resource = res['StackResourceDetail']

    metadata = _get_metadata_for_role(stack_arn, resource, policy_name, policy_metadata_filter)
    if metadata is None: 
        return
...
def _get_metadata_for_role(stack_arn, resource, policy_name, policy_metadata_filter):
